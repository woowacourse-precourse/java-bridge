...
public class InputViewValidator {
    ...
    public static void checkMovingCommandSize(final String command) {
        if (command.length() != COMMAND_SIZE) {
            UserInputException.invalidMovingCommandSize();
        }
    }

    public static void checkMovingCommandUpperCase(final String userInput) {
        for (int i = 0; i < userInput.length(); ++i) {
