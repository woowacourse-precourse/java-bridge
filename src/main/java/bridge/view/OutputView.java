...
public class OutputView {
    ...
    public void printMap(List<String> bridgeResult) {
        ...
        for (int i = 1; i < bridgeResult.size(); i++) {
            System.out.println(" | ");
            System.out.println(bridgeResult.get(i));
        }
        System.out.println(" ]");
    }
     ...
     * 출력을 위해 필요한 메서드의 인자(parameter)는 자유롭게 추가하거나 변경할 수 있다.
     */
    public void printResult(List<String> bridgeResult, String gameResult, int tryNumber) {
        System.out.println(GAME_RESULT_MESSAGE);
        printMap(bridgeResult);
        System.out.println(SUCCESS_OR_FAILURE_MESSAGE + gameResult);
