...
public class OutputView {
     ...
     * 출력을 위해 필요한 메서드의 인자(parameter)는 자유롭게 추가하거나 변경할 수 있다.
     */
    public static void printResult(Bridge bridge, User user) {
        printLine(MessageOutput.RESULT_STATEMENT);
        printMap(bridge, user);
        print(MessageOutput.RESULT_WHETHER_SUCCESS);
        printLine(BridgeCalculator.calculateSuccessOrFail(user.getIsSuccess()));
        print(MessageOutput.RESULT_TRY_COUNT);
        print(Integer.toString(user.getTotalTry()));
    }
}
