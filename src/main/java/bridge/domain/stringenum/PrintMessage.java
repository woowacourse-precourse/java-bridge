...
public enum PrintMessage {
    ...
    MOVE(String.format("이동할 칸을 선택해주세요. (위: %s, 아래: %s)", MovePosition.UPPER.getPosition(), MovePosition.LOWER.getPosition())),
    RESTART(String.format("게임을 다시 시도할지 여부를 입력해주세요. (재시도: %s, 종료: %s)", Restart.RESTART.getCommand(), Restart.END.getCommand())),
    RESULT("게임 성공 여부: %s"),
    TRY("총 시도한 횟수: %d"),
    END("최종 게임 결과"),
    SUCCESS("성공"),
    ...
    public String printMessage(int tryCount) {
