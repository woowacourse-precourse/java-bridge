...
public class Application {
    ...
    public static void 기능_테스트() {
        ...
        while (true) { // 게임 반복
            for (int i = 0; i < randomBridge.size(); i++) input.add(inputView.readMoving());
            decision = bridgeGame.move(randomBridge,input);
            if (decision != -1) break; // 게임 정답이 맞음
            retrySelect = bridgeGame.retry(inputView.readGameCommand());
            if(retrySelect == -1) break; // 재시작 안함
        }
        printResult(decision);
    }

    public static void printResult(int decision) {
