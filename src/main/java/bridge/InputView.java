...
public class InputView {
    ...
    private static String bridgeSizeNonIntegerErrorMessage = "[ERROR] 문자를 입력하셨습니다.";
    private static String bridgeSizeWithinRangeErrorMessage = "[ERROR] 다리 길이는 3부터 20사이의 숫자여야 합니다.";
    ...
    public Integer readBridgeSize() {
        try {
            ...
            validateBridgeSizeNonInteger(size);
            validateBridgeSizeWithinRange(size);
    ...
    private void validateBridgeSizeNonInteger(String size) {
        if (!size.chars().allMatch((Character::isDigit))) {
            System.out.println(bridgeSizeNonIntegerErrorMessage);
            throw new ArithmeticException(bridgeSizeNonIntegerErrorMessage);
        }
    }

    private void validateBridgeSizeWithinRange(String size) {
        if (Integer.parseInt(size) < minimumSize || Integer.parseInt(size) > maximumSize) {
            System.out.println(bridgeSizeWithinRangeErrorMessage);
            throw new ArithmeticException(bridgeSizeWithinRangeErrorMessage);
        }
    }
