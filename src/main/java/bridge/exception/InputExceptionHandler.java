...
import static bridge.domain.BridgeDirection.DOWN;
import static bridge.domain.BridgeDirection.UP;
...
public class InputExceptionHandler {
    ...
    public static String checkMovingCommand(String input) {
        if (UP.getDirection().equals(input)){
            return input;
        }
        if(DOWN.getDirection().equals(input)){
            return input;
        }
        throw new IllegalArgumentException(NOT_MOVING_COMMAND);
    }
