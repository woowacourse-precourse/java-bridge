...
public class OutputView {
    ...
    private static final String SELECT_PANEL_REQUEST_MESSAGE = "\n이동할 칸을 선택해주세요. (위: U, 아래: D)";
    private static final String RESTART_QUESTION_MESSAGE = "\n게임을 다시 시도할지 여부를 입력해주세요. (재시도: R, 종료: Q)";
    ...
    private String generateBridgeString(List<String> bridge, List<String> input, Direction direction) {
        ...
        for(int i = 0; i < input.size(); i++){
            ...
            if(i < input.size()-1)builder.append(PANEL_SEPARATOR);
        }
    ...
    public void printSelectDirectionRequest(){
        System.out.println(SELECT_PANEL_REQUEST_MESSAGE);
    }
    /**
    ...
    public void printResult(boolean isSuccess, int tryCount) {
        ...
        if(isSuccess)System.out.println("성공");
        if(!isSuccess)System.out.println("실패");
        System.out.println(NUMBER_OF_TRY+tryCount);
    }
