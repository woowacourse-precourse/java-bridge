...
public class OutputView {
    ...
    public void printMap(int num, List<String> bridge, int isAnswer) {
        ...
        printManyValueList(num, bridge, isAnswer);
        return;
    }
    ...
    public void printResult(int num, List<String> bridge, int isAnswer) {
        ...
        System.out.println("");
        if(isAnswer == 0 ) System.out.println("게임 성공 여부: 실패");
        if(isAnswer == 1 ) System.out.println("게임 성공 여부: 성공");
    }

    public void printManyValueList(int num, List<String> bridge, int isAnswer) {
        if(isAnswer == 0) {
            printIsFailedValueList(bridge, num);
            return;
        }
        if(isAnswer == 1) {
            printIsSuccessValueList(bridge, num);
            return;
        }
    }

    public void printIsSuccessValueList(List<String> bridge, int num) {
    ...
    public void printIsFailedValueList(List<String> bridge, int num) {
        List<String> upString = new ArrayList<>();
        List<String> downString = new ArrayList<>();
    ...
    public void printOneValue(List<String> bridge, int isAnswer) {
        ...
        if(isAnswer == 1) checkValue(bridge.get(0).charAt(0), upString, downString);
        if(isAnswer == 0) checkFailedValue(bridge.get(0).charAt(0), upString, downString);
        System.out.println("[ " + String.join(",",upString).replace(",", "") + "]");
        System.out.println("[ " + String.join(",",downString).replace(",","")+ "]");
    }
