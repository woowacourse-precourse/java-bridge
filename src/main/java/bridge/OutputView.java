...
public class OutputView {
     ...
     * 출력을 위해 필요한 메서드의 인자(parameter)는 자유롭게 추가하거나 변경할 수 있다.
     */
    public void printResult(List<String> bridge, int cnt,int trial) {
        System.out.println("최종 게임 결과");
        printMap(bridge, cnt, cnt == bridge.size());
        if(cnt == bridge.size()) System.out.println("게임 성공 여부: 성공");
        if(cnt != bridge.size()) System.out.println("게임 성공 여부: 실패");
