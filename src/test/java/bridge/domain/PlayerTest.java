...
class PlayerTest {
    ...
    void 이동가능_여부에_따른_상태값_계산(Boolean isMoveble, String expected) {
        assertThat(player.calculateStatusAfterMove(isMoveble)).isEqualTo(expected);
    }

    @DisplayName("사용자가 다리를 건넌 후의 상태를 테스트한다.")
    @ParameterizedTest
    @MethodSource("parametersProvider")
    void 사용자가_다리를_건넌_후_움직임_상태(String moves, Boolean isMoveble, String expectedUp, String expectedDown) {
        for (String move : moves.split("")) {
            Movement userMovement = Movement.valueOf(move);
            player.crossOverBridge(userMovement, isMoveble);
        }
