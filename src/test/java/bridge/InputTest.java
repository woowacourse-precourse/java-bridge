...
public class InputTest {
    ...
    void 다리의_길이가_범위에_없는_경우(String input) {
        Assertions.assertThatThrownBy(() -> validateBridgeSize(input))
    ...
    void 이동할_칸의_입력_값이_틀린_경우(String input) {
        Assertions.assertThatThrownBy(() -> validateMoveInput(input))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void 재시작_여부_입력_값이_틀린_경우(String input) {
        Assertions.assertThatThrownBy(() -> validateCommand(input))
                .isInstanceOf(IllegalArgumentException.class)
