...
2. BridgeGame 클래스에 게임 시작, 게임 다시 시작 , 입력 값  할떄 쓰이는 UI를 만든다.
...
6. inputView 클래스를 생성해서 입력 받는 값들을 정의 한후 반환 (다리 길이, 입력값 ,게임 시작 유뮤)+ 예외처리(3~20,문자 값 올바른지)
7. UserBridge 클래스를 만들어 user가 선택한 입력 값에 따라 다리 생성 값을 반환하는 클래스 구현
8. OutView UserBridge 클래스에서 받은 반환 값으로 출력 UI생성
9. Game 클래스를 만들고 Bridge는 변하지 않는 벰버변수로 두고  게임 진행하는 메서드 구현
