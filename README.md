...
1. 지나온 다리 상태를 알려주는 변수
    - 다리 몇개째를 건너고 있는지 체크하는 변수(Integer)가 필요하다.  
    - 다리를 잘못 디디는 순간 게임이 초기화되거나 끝나므로, 어디까지 통과했는지만 저장하고 다리를 잘못 디디는 순간 다시 0 으로 초기화할 수 있다.
    - 다리를 건널때마다, BridgMaker 객체는 다리를 몇개째 건너고 있는지 카운팅할 수 있다.
    - 이 변수를 통해, 전부 통과했는지 아닌지 현황을 파악할 수 있다.

2. Output(UI) 가 출력할 수 있도록, 건넌 다리 List<String> 반환하는 기능
    - 이미 건넌 다리를 파악해 출력할 수 있게, U 와 D 로 구성된 징검다리 리스트를 반환해야 한다.
    - 이 기능을 통해, UI 에서 이미 건넌 다리들을 출력할 수 있다.    

3. 징검다리 Up/Down 선택시, 맞는지 아닌지 처리하는 부분
    - 선택이 맞았다면, 다리 진행 변수를 하나 올리고 true를 반환
    - 선택이 틀렸다면, 다리 진행 변수를 0으로 바꾸고 false를 반환

4. 다리를 다 건넜는지 아닌지에 대한 정보 처리 부분
