...
- [x] 사용자가 잘못된 값을 입력할 경우 `IllegalArgumentException`를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 그 부분부터 입력을 다시 받는다.
  - [x] 다리 생성시 올바른 값이 아니면 예외 처리한다.
    - [x] 다리길이 입력값은 3 이상 20 이하의 숫자를 입력할 수 있다.
    - [x] 숫자만 입력이 가능하다.
  - [x] 플레이어 이동시, U(위 칸)와 D(아래 칸) 중 하나의 문자를 입력할 수 있으며 올바른 값이 아니면 예외 처리한다.
  - [x] 게임 재시작 및 종료 입력을 받을시, R(재시작)과 Q(종료) 중 하나의 문자를 입력할 수 있으며 올바른 값이 아니면 예외 처리한다.