...
6. BridgeGame 클래스에 이동이나 재시작 할떄 쓰이는 메서드를 만든다.(upBridge,downBridge 두개 배열을 두고 유저값을 저장해주고 반환하면서 구현)
7. BridgeGame 클래스에 재시작 카운트 하는 변수와 성공인지 아닌지 체크 변수 추가 (move시에 일치 하지 않으면 성공 아니게함 + 재시작시 성공으로 바꾸고 다시 시작)
7. OutView BridgeGame 클래스에서 받은 반환 값으로 출력 UI생성
