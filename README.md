...
- 다리 만들기
  - 다리 개수를 입력 받는다.
  - 3이상 20이하 숫자가 아니면 예외를 반환하고 입력값을 다시 받는다.
  - 입력값에 맞는 다리(객체)를 생성한다.
- 다리 건너기
  - 이동 객체를 생성한다.
  - 이동할 방향을 입력 받는다.
    - 'U'나 'D'가 아니면 예외를 발생시키고 입력값을 다시 받는다.
  - 다리를 건넌다.
  - 옳은 다리를 건넜는지 확인한다.
  - 중간 결과를 저장한다.
  - 중간 결과를 출력한다.
- 재도전 및 최종 결과 출력
  - 올바른 다리를 건널 경우 이동할 방향을 다시 입력 받는다.
  - 올바르지 않는 다리를 건널 경우
    - 재도전 여부를 입력 받는다 
      - 'R'이나 'Q'가 아니면 예외를 발생시키고 입력값을 다시 받는다 
      - 재도전 시 이동 및 결과 객체를 초기화하고 시행 횟수를 1만큼 증가시킨다.
         - 이동할 방향을 다시 입력 받는다.
  - 최종 결과를 반환한다.
