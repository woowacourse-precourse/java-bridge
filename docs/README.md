...
- [V] 사용자의 입력값이 이동 가능한 다리의 값과 일치하지 않을 경우 이동에 실패했음을 출력한다.

### 리팩토링
- [V] printMap 메서드 분할

## 3. 다리를 끝까지 건너지 못하면 재시도를 묻고, 끝까지 건너면 결과를 출력한다.
- [V] 다리를 끝까지 건너지 못했을 경우 재시도 여부를 묻는다.
- [V] 재시도 여부를 입력받는다.
- [V] 재시도 입력값이 올바른지 검증한다.
- [V] 입력값이 올바르지 않을 경우 [ERROR]로 시작하는 메시지를 출력한다.
- [V] 재시도를 원하면 게임을 다시 시작한다.
- [V] 종료를 선택하면 최종게임 결과를 출력한다.