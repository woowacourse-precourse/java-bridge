...
1. GameController
    ...
    - [x] 게임 재시작 or 종료 → checkGameStatus
        1. 게임 재시작인지 종료인지 확인
        2. 결과에 따라서 게임 재시작 과정이나 게임 종료 과정 호출
    - [x] 게임 재시작 → retryGame
        1. 재시작 여부 리턴
        2. 결과에 따라서 게임 다시 시작이나 게임 종료 호출
    - [ ] 게임 종료 → endGame
        1. 게임 종료 결과 출력
2.  InputController
...
- [ ]  메서드의 길이가 10라인을 넘지 않았는가?
- [ ]  메서드의 파라미터 개수는 최대 3개까지만 사용하였는가?
- [ ]  InputView
...
- [ ]  MVC패턴의 규칙을 지켰는가?
- [ ]  final 키워드를 사용해 값의 변경을 막았는가?
- [ ]  인스턴스 변수는 private으로 구현했는가?
- [ ]  객체는 객체스럽게 사용했는가?