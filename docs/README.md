...
3. 함수(또는 메서드)가 한 가지 일만 하도록 최대한 작게 만들어라 check!
4. 본인이 정리한 기능 목록이 정상 동작함을 테스트 코드로 확인 check!
5. else 예약어를 쓰지 않는다 check!
6. 도메인 로직에 단위 테스트를 구현 check!
7. 함수(또는 메서드)의 길이가 10라인을 넘어가지 않도록 구현 check!
8. 메서드의 파라미터 개수는 최대 3개까지만 허용 check!
9. 클래스 관련 요구사항 check!
    1. InputView 클래스에서만 camp.nextstep.edu.missionutils.Console 의 readLine() 메서드를 이용해 사용자의 입력을 받을 수 있다.
...
- [X] 재시작시, 기존 다리 재사용하기
- [X] 다리 건너기 성공 &rarr; 게임 종료
- [X] 게임 종료시, 성공 여부와 시도 횟수 출력하기
- [X] 예외처리 &rarr; `[ERROR]`와 같이 오류 메세지 출력 후, 그 부분부터 다시 입력받기
-------------------
### 0. 생각에 없던 기능 목록
#### 추가한 기능 목록
###### OutputView
...
###### Application
    루프에 bridge와 moveResult의 크기를 비교
    moveResult의 크기가 bridge의 크기보다 작을 때만 루프 반복
    루프를 빠져나오는 2가지 case
        1. 모든 다리를 건너서 루프를 빠져나옴. 
        2. 다리를 건너다가 실패하고 "Q"를 입력하여 루프를 빠져나옴.
-------------------
...
###### BridgeGame
    1. countOfTry 변수 추가
        시도 횟수를 저장하여, 출력 시 이용
    2. printResult()
        세부 내용 작성: Bridge game 객체를 받아와, 성공 여부와 시도 횟수를 출력
-------------------
