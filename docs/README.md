...
|7| 상황 출력이 올바른가? - 성공 하였을 때 상황에 맞게 잘 출력되는가?|☑️|
|8| 상황 출력이 올바른가? - 실패 하였을 때 상황에 맞게 잘 출력되는가?|☑️|
|9| 상황 출력이 올바른가? -  2번 만에 성공 하였을 때 잘 출력 되는가?|☑️|
|10| 게임이 끝나면 다시 재시작 여부를 묻는가? |☑️|
|11| 재시작시 다시 처음 다리가 생성된 상태로 돌아가는가? |☑️|
|12| 종료시 최종 결과 값들이 잘 출력 되는가? |☑️|
|13| 사용자가 잘된 값을 입력할 경우 IllegalArgumentException를 발생시키는가?|☑️| |☑️|
