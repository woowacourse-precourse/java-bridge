...
### 1. 다리의 길이를 입력
    - 입력받은 길이의 다리를 랜덤하게 생성
    - 3~20 범위의 정수가 아니면 다시 입력받는다.
### 2. 위/아래 움직임을 입력
    - 위쪽 다리, 아래쪽 다리 중 어디로 갈지 입력받는다.
    - U, 혹은 D가 아니면 다시 입력 받는다.
### 3. 다리의 형태를 출력
    - 선택된 위치는 이동 가능, 불가능에 따라 O, X로
    - 선택되지 않은 위치는 공백으로 표시된다.
### 4-1. 이동 성공 시
    - 2번으로 돌아간다.
### 4-2. 이동 실패 시
    - 5번으로 이동한다.
### 5. 게임 재시작/종료를 입력
    - 재시작을 원하면 R을 입력, 2번으로 돌아간다.
    - 종료를 원하면 Q를 입력, 6번으로 이동한다.
### 6. 게임 종료, 결과 출력
    - 최종적으로 이동한 결과를 출력한다.
    - 게임 성공/실패 여부와 시도 횟수를 출력한다.
    - 다리 건너기 게임 종료
