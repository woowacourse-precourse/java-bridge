...
4. [ ] **건널 수 있는 다리 무작위로 설정**
   1. [ ] **Utils.makeAnswerBridge() 생성, 구현**
      1. [ ] **BridgeRandomNumberGenerator의 객체 생성**
      2. [ ] **BridgeMaker 객체 생성, 위에서 생성한 BridgeRandomNumberGenerator의 객체를 생성자 파라미터로 넘겨 줌**
      3. [ ] **BridgeMaker.makeBridge() 구현**
         1. [ ] **BridgeRandomNumberGenerator.generate()를 size만큼 호출해서 0과 1 중 하나의 랜덤 값 size개 반환받기**
         2. [ ] **List<String\> 생성, BridgeRandomNumberGenerator.generate()의 결과를 "U", "D"로 변환해 리스트에 저장**
         3. [ ] **정답이 될 다리의 모양이 저장된 리스트를 반환**
