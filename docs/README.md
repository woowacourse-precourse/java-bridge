...
|3| 다리의 상황에 맞게 입력한 이동할 칸과 비교가 잘 되는가? - 위층 다리를 건널 차례일 때 U를 입력하면 다리가 잘 건너지는가?|✅|
|4| 다리의 상황에 맞게 입력한 이동할 칸과 비교가 잘 되는가? - 아래층 다리를 건널 차례일 때 D를 입력하면 다리가 잘 건너지는가?|✅️|
|5| 다리의 상황에 맞게 입력한 이동할 칸과 비교가 잘 되는가? - 위층 다리를 건널 차례일 때 D를 입력하면 다리 건너기가 끝나는가?|✅️|
|6| 다리의 상황에 맞게 입력한 이동할 칸과 비교가 잘 되는가? - 아래층 다리를 건널 차례일 때 U를 입력하면 다리 건너기가 끝나는가?|✅️|
|7| 상황 출력이 올바른가? - 성공 하였을 때 상황에 맞게 잘 출력되는가?|✅️️|
|8| 상황 출력이 올바른가? - 실패 하였을 때 상황에 맞게 잘 출력되는가?|✅️️|
|9| 상황 출력이 올바른가? -  2번 만에 성공 하였을 때 잘 출력 되는가?|✅️️|
|10| 게임이 끝나면 다시 재시작 여부를 묻는가? |✅️|
|11| 재 시작시 다시 처음 다리가 생성된 상태로 돌아가는가? |✅️|
|12| 종료시 최종 결과 값들이 잘 출력 되는가? - 게임이 재시작되어 반복 되어도 잘 돌아가고 성공시 잘 돌아가는가?|✅️|
|13| 종료시 최종 결과 값들이 잘 출력 되는가? - 게임 실패 후 재시작되어 반복 되어도 잘 돌아가고 성공시 잘 돌아가는가?|✅️|
|14| 종료시 최종 결과 값들이 잘 출력 되는가? - 게임이 재시작되어 반복 되어도 잘 돌아가고 실패시 잘 돌아가는가?|✅️|
|15| 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException를 발생시키는가? <br> - 다리 길이 입력에서 3이하 숫자를 입력하였을 때 "[ERROR]"가 출력되고 다시 입력을 받는가?|✅️️|
|16| 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException를 발생시키는가? <br> - 다리 길이 입력에서 40이상 숫자를 입력하였을 때 "[ERROR]"가 출력되고 다시 입력을 받는가?|✅️️|
|17| 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException를 발생시키는가? <br> - 다리 길이 입력에서 아무 것도 입력하지 않았을 때 "[ERROR]"가 출력되고 다시 입력을 받는가?|✅️️|
|18| 사용자가 잘못된 값을 입력할 경우 NumberFormatException 발생시키는가? <br> - 다리 길이 입력에서 문자를 입력했을 때 "[ERROR]"가 출력되고 다시 입력을 받는가?|✅️️|
|19| 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException를 발생시키는가? <br> - 사용자가 이동할 칸 입력에서 U, D가 아닌 다른 문자를 입력 하였을 때"[ERROR]"가 출력되고 다시 입력을 받는가?|✅️️|
|20| 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException를 발생시키는가? <br> - 사용자가 이동할 칸 입력에서 아무 것도 입력하지 않았을 때"[ERROR]"가 출력되고 다시 입력을 받는가?|✅️️|
|21| 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException를 발생시키는가? <br> - 재시도 여부 입력에서 R, Q가 아닌 다른 문자를 입력하였을 때 "[ERROR]"가 출력되고 다시 입력을 받는가?|✅️️|
