...
3. [ ] **다리 생성 (매 라운드마다 사용자가 건넌 다리를 출력하기 위함)**
   1. [ ] **사용자가 건넌 다리 정보를 저장할 UsersBridgeCrossStatus 클래스 생성**
         ...
         3. [ ] **currentBridge 반환**
   2. [ ] BridgeGame 클래스에 makeNewBridge()라는 메서드 추가
      1. [ ] 메서드 내부에서 BridgeNumberGeneratorByUserInput의 객체 생성
      2. [ ] 메서드 내부에서 BridgeMaker의 객체 생성, 위에서 생성한 BridgeNumberGeneratorByUserInput의 객체를 생성자 매개변수로 넘겨주기
      3. [ ] BridgeMaker 객체 반환
