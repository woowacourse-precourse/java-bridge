...
1. [ ] 다리를 모두 건너거나 실패할 때까지 게임 진행
   ...
   3. [ ] **이동 가능한 칸이라면 해당 칸에 O, 가능하지 않은 칸이라면 X 저장**
      1. [ ] **UsersBridgeCrossStatus의 객체에 addNewCrossingResult() 메서드 추가**
         1. [ ] **해당 이동 결과가 마지막 다리에 대한 결과가 아니라면 "["와  "]"가 저장되어 있는 currentBridge 사이에 " ", "(O 또는 X)", " ", "|" 추가**
         2. [ ] **해당 이동 결과가 마지막 다리에 대한 결과라면 "["와 "]"가 저장되어 있는 currentBridge 사이에 " ", "(O 또는 X)", " " 추가**
         3. [ ] **currentBridge 반환**
   4. [ ] 현재까지 건넌 다리 모두 출력
