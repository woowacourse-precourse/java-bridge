...
- [ ] 게임 시작 문구를 출력한다.
- [ ] 다리 길이를 입력 받을 수 있다.
   - [ ] 다리 길이가 정수인가 아니면 아니면 IllegalArgumentException을 발생시킨다.
   - [ ] 다리 길이가 3이상 20 이하가 아니면 아니면 IllegalArgumentException을 발생시킨다.
- [ ] 다리 길이만큼 다리를 생성할 수 있다.
   - [ ] 다리를 생성할 때 건널 수 있는 칸은 0과 1 중 무작위 값을 이용해서 정한다.
   - [ ] 무작위 값이 0인 경우 아래 칸, 1인 경우 위 칸이 건널 수 있는 칸이 된다.
   - [ ] 위 칸을 건널 수 있는 경우 U, 아래 칸을 건널 수 있는 경우 D값으로 나타낸다.
- [ ] 플레이어가 U(위 칸), D(아래 칸) 중 하나를 입력하여 이동할 칸을 선택할 수 있다.
   - [ ] U(위 칸)와 D(아래 칸) 중 하나의 문자 값이 아니면 IllegalArgumentException을 발생시킨다.
