...
### 다리 생성
- [] 위아래 두 칸으로 이루어진 다리를 입력받은 길이만큼 랜덤으로 생성한다.
  - 0과 1 중 무작위 값을 이용해서 생성된다.
  - [] 위칸을 건널 수 있는 경우 U, 아래 칸을 건널 수 있는 경우 D로 나타낸다.

### 이동할 칸 선택
- [] 사용자에게 `U`(위 칸)과 `D`(아래 칸) 중 하나를 입력받는다.
